...
from utils import create_training_data, create_training_data_biased, calculate_ROC, NUM_OF_VERTICES, get_log_location
...
def compute_all_properties_of_list(all_sparse, vlist, data_source):
    ...
    for ii in range(len(vlist)):
        ...
        if ii % 10 ** 4 == 0:
            print('    compute_all_properties_of_list progress: (', time.time() - time_start, 'sec) ', ii / 10 ** 6,
                  'M/', len(vlist) / 10 ** 6, 'M')
...
def link_prediction_semnet(full_dynamic_graph_sparse, unconnected_vertex_pairs, year_start, years_delta,
    ...
    train_dynamic_graph_sparse, train_edges_for_checking, train_edges_solution = create_training_data_biased(
        full_dynamic_graph_sparse, year_start - years_delta, years_delta, min_edges=min_edges, edges_used=edges_used,
